...
In some game providers there may be occurances where new `round_id` is made for each consecutive action on the slotmachine, for example when bonus game won on some providers this might be sent as individual game rounds and will thus only send `credit` action. 
